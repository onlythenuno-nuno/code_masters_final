...
if (isset($_GET['remover']) && is_numeric($_GET['remover'])) {
    ...
    if ($result->num_rows > 0) {
        // Desativar ou excluir o aluno (dependendo da sua política)
...
$registros_por_pagina = 40; // Alunos por página
...
if (!empty($filtro_nome)) {
    $where .= " AND nome_aluno LIKE '%" . mysqli_real_escape_string($conn, $filtro_nome) . "%'";
}
if (!empty($filtro_email)) {
    $where .= " AND email_aluno LIKE '%" . mysqli_real_escape_string($conn, $filtro_email) . "%'";
}
...
// Ajustar página atual se ultrapassar o total de páginas
                        ...
                        <tbody>
                            ...
                            if (mysqli_num_rows($alunos) > 0) {
                                while ($aluno = mysqli_fetch_assoc($alunos)) {
                                    ...
                                    echo '<a href="#" onclick="confirmarExclusao(' . $aluno['id_aluno'] . ')" class="action-btn delete-btn small">Remover</a>';
                                    echo '</td>';
                                    echo '</tr>';
                                }
                            } else {
